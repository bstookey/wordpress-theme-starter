...
class IP_Nav_Social_Walker extends Walker_Nav_Menu
{
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        $slug  = !empty($item->title) ? sanitize_title($item->title) . ' ' : '';
        $class_names = $value = '';
        $classes = empty($item->classes) ? array() : (array) $item->classes;
        $classes[] = 'menu-item-' . $item->ID;
        $class_names = join(' ', apply_filters('nav_menu_css_class', array_filter($classes), $item, $args));
        $class_names = $class_names ? ' class="' . $slug . esc_attr($class_names) . '"' : '';
        ...
        if (strpos($item->url, 'facebook') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-facebook"></i>';
            $item_output .= '</a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'twitter') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-twitter">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'linkedin') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-linkedin">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'instagram') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-instagram">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'youtube') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-youtube">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'pinterest') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-pinterest">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'reddit') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-reddit">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'snapchat') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-snapchat">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'vimeo') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-vimeo">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'whatsapp') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-whatsapp">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'tiktok') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-tiktok">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'telegram') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-telegram">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'quora') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-quora">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'skype') !== false) {
            $item_output .= '<a class="' . $slug . '"' . $attributes . '><i class="fab fa-skype">';
            $item_output .= '</i></a>';
            $item_output .= $args->after;
        }
